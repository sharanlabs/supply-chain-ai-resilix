...
export function PipelineWalk({
  outcome,
  cell
}: {
  outcome: CustomsDefenseOutcome;
  cell: MatrixCell;
}) {
  const { packet, disposition, namedGaps } = outcome;
  const exhibitCount = packet.exhibitAudit.length;
  const injectionSignals = [...new Set(packet.exhibitAudit.flatMap((e) => e.injectionSignals))];
  ...
  return (
    ...
    >
      ...
      <p className="mt-1.5 max-w-[68ch] text-[0.875rem] leading-relaxed text-ink-muted">
        Four deterministic stages, run in order. The disposition is decided in code from structured
        fields — never from exhibit prose.
      </p>

      {/* The timeline spine is a REAL element, not a `before:` pseudo: axe's
          color-contrast rule cannot determine text backgrounds under an ancestor
          pseudo-element and reported every stage figure `incomplete`. A real,
          aria-hidden span is visually identical and fully decidable -- and it
          lives on a wrapper div (not inside the <ol>) so the list keeps only
          <li> children (axe `list` rule, WCAG 1.3.1). */}
      <div className="relative mt-6">
        <span aria-hidden="true" className="absolute top-1 bottom-1 left-3 w-px bg-line" />
        <ol className="flex flex-col gap-6">
          <Stage icon={ShieldCheck} label="1 · Exhibit quarantine" tone="accent">
            <span className="tnum font-medium text-ink">{exhibitCount}</span> exhibits examined
            under per-document quarantine. Each body is third-party free text — it is held at the
            boundary, and only enum-typed structured fields cross into the analysis.
            {injectionSignals.length > 0 ? (
              <span className="mt-2 flex flex-wrap items-center gap-1.5">
                <span className="text-[0.8125rem] text-ink-faint">
                  Flagged for audit, quarantine held:
                </span>
          ...
          <Stage
            icon={Scale}
            label="2 · Evidence sufficiency"
            tone={isProceed ? "accent" : "caution"}
          >
            {isProceed ? (
              <>
                Sufficiency predicates passed: every load-bearing record is present and consistent
                with the declared origin
                {cell.deadline !== "LAPSED" ? ", and the disclosure window is open" : ""}. The
                engine may proceed to a packet.
              </>
            ) : (
              <>
          ...
          <Stage icon={Hash} label="3 · Cited figures" tone="accent">
            {isProceed ? (
              <>
                <span className="tnum font-medium text-ink">{figureCount}</span> figures bound, each
                to a deterministic tool return — the entry scoper, the penalty calculator, and the
                deadline clocks. No number originates in prose; a produce-time citation guard fails
                closed on any uncited numeral.
              </>
            ) : (
              <>
                No exposure figures are derived for a withheld disclosure. Only the quarantine count
                is cited — the same produce-time guard still fails closed on any uncited numeral in
                the refusal memo.
              </>
            )}
          </Stage>
          ...
          >
            {isProceed ? (
              <>
                A filing-grade support packet was assembled from the bound figures — shown below,
                draft until counsel approves.
              </>
            ) : (
              <>
                The engine withheld the disclosure. The refusal below names each gap plainly — there
