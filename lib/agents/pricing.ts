...
// The pinned price-list version. Bump this (and the prices) together when Google
// re-prices; the stamp on each run says which list was in force. Pure string id.
  ...
  if (!price) {
    throw new Error(
      `costUsd: unknown model "${key}" -- not in the pinned pricing table ` +
        `(${PRICING_VERSION}). Known models: ${Object.keys(GEMINI_PRICING).join(", ")}. ` +
        `Add it to GEMINI_PRICING before billing a live call on it.`
    );
  }
