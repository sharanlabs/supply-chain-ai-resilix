...
- **Provisional verdict.** BUILD-NEW. The **deterministic graders** (Atlas math, supplier-ID existence, citation-coverage, off-taxonomy `OTHER_UNMAPPED`, simulator arithmetic) are buildable **pre-key** against fixtures; the **LLM-judge + live-mode** parts DEFER to post-key.
