...
- **The unsupported-claims judge is same-family (Gemini grading Gemini).** It is a *secondary* semantic check — the deterministic graders (numeral→source citation, entity/URL existence, injection quarantine) and atomic human approval are the primary gates; the judge is fail-closed and off by default. A same-family judge carries a documented self-preference bias; a cross-family (non-Gemini) judge is the path that removes it, gated on a billed recalibration. Decision recorded in [docs/adr/0002-same-family-llm-judge.md](docs/adr/0002-same-family-llm-judge.md).
