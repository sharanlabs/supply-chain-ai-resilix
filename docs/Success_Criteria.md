...
| LLM calls per pipeline run | 3 (+2 retry reserve) on the deterministic **waterfall** (the opt-out / key-off demo path). With the Investigator loop default-on (2026-06-29), a live run's call count is **variable** — the tool-using loop decides what to call, hard-stopped by the $5/run budget cap rather than a fixed count. | Counted from persisted agent runs; the loop's per-run cost + step cap are the bound |
